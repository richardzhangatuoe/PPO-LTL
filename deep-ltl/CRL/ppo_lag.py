...
class PPOLagrangian(PPO):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # pop custom kwargs so SB3.PPO does not see them
        self.cost_limit = float(cost_limit)
        self.lagrangian_lr = float(lagrangian_lr)
        self.normalize_cost_adv = bool(normalize_cost_adv)
        self.cost_gamma = float(kwargs.pop("cost_gamma", cost_gamma))
        self.cost_gae_lambda = float(kwargs.pop("cost_gae_lambda", cost_gae_lambda))
        kwargs.pop("cost_limit", None)
        kwargs.pop("lagrangian_lr", None)
        kwargs.pop("normalize_cost_adv", None)
        super().__init__(*args, **kwargs)
